    ...
    <tr className={video.isDeleted ? "opacity-50" : ""}>
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex justify-end gap-2">
          {editing ? (
            <>
              <button disabled={busy} onClick={save} className="rounded bg-brand-600 px-3 py-1 text-xs text-white hover:bg-brand-500">
                Save
              </button>
              <button onClick={() => setEditing(false)} className="rounded bg-zinc-800 px-3 py-1 text-xs text-zinc-300">
                Cancel
              </button>
            </>
          ) : (
            <>
              <button onClick={() => setEditing(true)} className="rounded bg-zinc-800 px-3 py-1 text-xs text-zinc-300 hover:bg-zinc-700">
                Edit
              </button>
              <button
                disabled={busy}
