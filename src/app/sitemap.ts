...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...staticRoutes,
    ...videos.map((v) => ({ url: `${base}/videos/${v.slug}`, lastModified: v.updatedAt, changeFrequency: "weekly" as const, priority: 0.8 })),
    ...pornstars.map((p) => ({ url: `${base}/pornstars/${p.slug}`, changeFrequency: "weekly" as const, priority: 0.6 })),
    ...creators.map((c) => ({ url: `${base}/creators/${c.slug}`, changeFrequency: "weekly" as const, priority: 0.6 })),
    ...tags.map((t) => ({ url: `${base}/tags/${t.slug}`, changeFrequency: "weekly" as const, priority: 0.5 })),
  ];
}
